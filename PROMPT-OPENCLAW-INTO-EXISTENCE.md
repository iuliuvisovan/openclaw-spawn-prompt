...
20. **Locally-patched plugins get wiped on auto-update**: Any direct edit to a file under `~/.claude/plugins/cache/<plugin>/<version>/` is silently overwritten when the plugin auto-updates -- the cache only keeps the current version, and old version directories are removed. No warning. Also note that the running bun process holds the previous code in memory until it restarts, so a freshly applied patch only takes effect after the next plugin restart. If you maintain a local patch (e.g., to the telegram plugin's `server.ts` for sendMediaGroup album support), verify it after every Claude or plugin update: `grep -l "<your-marker>" ~/.claude/plugins/cache/claude-plugins-official/<plugin>/*/server.ts`. If empty, reapply against the new version. Better long-term: keep the patch as a reusable script, not a manual edit, and upstream as a PR when possible.
